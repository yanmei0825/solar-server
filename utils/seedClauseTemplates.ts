...
export async function ensureClauseTemplatesSeeded(): Promise<void> {
  if (seedClauseTemplates.length === 0) return;

  await ClauseTemplate.bulkWrite(
    seedClauseTemplates.map((t) => ({
      updateOne: {
